...
namespace BusinessLogic
{
    public class MainBLL
    {
                ...
                using (db = new ApplicationDbContext())
                {
                    var tenders = db.Tender
                        ...
                        .Select(tender => new HomeTableDTO
                        {
                            ...
                            OpenDate = tender.OpenDate.ToString().Substring(0, 11),
                            CloseDate = tender.CloseDate.ToString().Substring(0, 11),
                        })
                        .ToList();
        ...
        public List<Report> GetReportForSellers(int Id)
        {
            try
            {
                using (db = new ApplicationDbContext())
                {
                    var reports = db.Tender
                        .Include(x => x.User)
                        .Where(x => x.StatusId == 3)
                        .Select(report => new Report
                        {
                            Id = report.Id,
                            TenderNo = report.TenderNo,
                            TenderOpen = report.OpenDate.ToString().Substring(0, 11),
                            TenderClosed = report.CloseDate.ToString().Substring(0, 11),
                        })
                        .ToList();
                    return reports;
                }
